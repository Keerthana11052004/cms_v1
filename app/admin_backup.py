...
def vendor_report_unit_wise():
    ...
    query = "SELECT name as vendor_name, unit, food_licence_path, agreement_date FROM vendors WHERE (purpose IS NULL OR purpose = %s OR purpose NOT LIKE %s)"
    count_query = "SELECT COUNT(*) as count FROM vendors WHERE (purpose IS NULL OR purpose = %s OR purpose NOT LIKE %s)"
    params = ['', 'Outsider:%']
    ...
    if request.args.get('vendor_name'):
        ...
        params.append(f"%{request.args.get('vendor_name')}%")
        count_params.append(f"%{request.args.get('vendor_name')}%")

    if request.args.get('unit'):
        where_conditions.append('unit = %s')
        params.append(request.args.get('unit'))
        count_params.append(request.args.get('unit'))
...
@admin_bp.route('/vendor_report_outsider_wise')
@login_required
def vendor_report_outsider_wise():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    query = "SELECT name as vendor_name, unit, purpose, count FROM vendors WHERE purpose LIKE 'Outsider:%'"
    count_query = "SELECT COUNT(*) as count FROM vendors WHERE purpose LIKE 'Outsider:%'"
    params = []
    count_params = []
    where_conditions = []
    ...
    return render_template('admin/vendor_report_outsider_wise.html',
                           vendors=rows,
                           pagination=pagination)
...
def add_user():
    ...
    if form.validate_on_submit():
        ...
        password_hash = hashlib.sha256(password.encode()).hexdigest()
        try:
            cur.execute("INSERT INTO employees (employee_id, name, email, password_hash, role_id, department_id, location_id) VALUES (%s, %s, %s, %s, %s, %s, %s)", (employee_id, name, email, password_hash, role_id, department_id, location_id))
            conn.commit()
...
@admin_bp.route('/add_vendor', methods=['GET', 'POST'])
@login_required
def add_vendor():
    form = VendorForm()
    if form.validate_on_submit():
        name = form.name.data
        unit = form.unit.data
        food_licence_path = form.food_licence_path.data
        agreement_date = form.agreement_date.data
        conn = get_db_connection()
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO vendors (name, unit, food_licence_path, agreement_date) VALUES (%s, %s, %s, %s)", (name, unit, food_licence_path, agreement_date))
            conn.commit()
            flash('Vendor added successfully!', 'success')
            return redirect(url_for('admin.add_vendor'))
        except IntegrityError:
...
@admin_bp.route('/add_outsider_meal_vendor', methods=['GET', 'POST'])
@login_required
def add_outsider_meal_vendor():
    ...
    if form.validate_on_submit():
        ...
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO vendors (name, unit, purpose, count) VALUES (%s, %s, %s, %s)", (name, unit, purpose, count))
            conn.commit()
            flash('Outsider Meal Vendor added successfully!', 'success')
            return redirect(url_for('admin.add_outsider_meal_vendor'))
        except IntegrityError:
            flash('Vendor already exists.', 'danger')
        finally:
            cur.close()
            conn.close()
    return render_template('admin/add_outsider_meal_vendor.html', form=form)
...
def add_menu():
    ...
    if form.validate_on_submit():
        ...
        menu_items = form.menu_items.data
        conn = get_db_connection()
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO menus (vendor_name, unit, shift, menu_date, menu_items) VALUES (%s, %s, %s, %s, %s)", (vendor_name, unit, shift, menu_date, menu_items))
            conn.commit()
...
def download_csv():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT employee_id, name, email, role_id, department_id, location_id FROM employees")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    ...
    writer.writerow(['Employee ID', 'Name', 'Email', 'Role ID', 'Department ID', 'Location ID'])
    for row in rows:
        writer.writerow(row)
...
@admin_bp.route('/download_csv_vendors')
@login_required
def download_csv_vendors():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT name, unit, purpose, count FROM vendors")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Name', 'Unit', 'Purpose', 'Count'])
    for row in rows:
        writer.writerow(row)
...
@admin_bp.route('/download_csv_menus')
@login_required
def download_csv_menus():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT vendor_name, unit, shift, menu_date, menu_items FROM menus")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Vendor Name', 'Unit', 'Shift', 'Menu Date', 'Menu Items'])
    for row in rows:
        writer.writerow(row)

    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=menus.csv'
    response.headers['Content-type'] = 'text/csv'
    return response

@admin_bp.route('/download_csv_bookings')
@login_required
def download_csv_bookings():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT employee_id, booking_date, shift, status FROM bookings")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Employee ID', 'Booking Date', 'Shift', 'Status'])
    for row in rows:
        writer.writerow(row)
...
@admin_bp.route('/download_csv_daily_unit_report')
@login_required
def download_csv_daily_unit_report():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Location', 'Total Bookings', 'Consumed Meals', 'Booked Meals'])
    for row in daily_reports:
        writer.writerow(row)

    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=daily_unit_report.csv'
    response.headers['Content-type'] = 'text/csv'
    return response

@admin_bp.route('/download_csv_monthly_unit_report')
@login_required
def download_csv_monthly_unit_report():
    if current_user.employee_id == 'a001' or not current_user.location:
        flash('Access denied: This report is for unit-specific admins only.', 'danger')
        return redirect(url_for('admin.dashboard'))
    ...
    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=monthly_unit_report.csv'
    response.headers['Content-type'] = 'text/csv'
    return response

@admin_bp.route('/download_csv_monthly_all_units_report')
@login_required
def download_csv_monthly_all_units_report():
    if current_user.employee_id != 'a001':
        flash('Access denied: Only the Master Admin can view this report.', 'danger')
        return redirect(url_for('admin.dashboard'))
    ...
    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=monthly_all_units_report.csv'
    response.headers['Content-type'] = 'text/csv'
    return response

@admin_bp.route('/download_csv_employee_reports')
@login_required
def download_csv_employee_reports():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Employee', 'Department', 'Location', 'Employee ID', 'Meals Booked', 'Meals Consumed'])
    for row in employees:
        writer.writerow(row)

    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=employee_reports.csv'
    response.headers['Content-type'] = 'text/csv'
    return response

@admin_bp.route('/download_csv_dept_location_reports')
@login_required
def download_csv_dept_location_reports():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Department', 'Location', 'Meals Booked', 'Meals Consumed'])
    for row in reports:
        writer.writerow(row)

    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=dept_location_reports.csv'
    response.headers['Content-type'] = 'text/csv'
    return response

@admin_bp.route('/download_csv_cost_subsidy')
@login_required
def download_csv_cost_subsidy():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('SELECT d.name FROM employees e JOIN departments d ON e.department_id = d.id WHERE e.id = %s', (current_user.id,))
    dept = cur.fetchone()
...
@admin_bp.route('/download_csv_vendor_report_unit_wise')
@login_required
def download_csv_vendor_report_unit_wise():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Vendor Name', 'Unit', 'Food Licence Path', 'Agreement Date'])
    for row in rows:
        writer.writerow(row)

    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=vendor_report_unit_wise.csv'
    response.headers['Content-type'] = 'text/csv'
    return response

@admin_bp.route('/download_csv_vendor_report_outsider_wise')
@login_required
def download_csv_vendor_report_outsider_wise():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Vendor Name', 'Unit', 'Purpose', 'Count'])
    for row in rows:
        writer.writerow(row)

    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=vendor_report_outsider_wise.csv'
    response.headers['Content-type'] = 'text/csv'
    return response
...
def download_csv_vendor_report_outsider_wise():
    ...
    total_count = cur.fetchone()['count']
    print(f"[DEBUG] Total vendor count: {total_count}")
    ...
    print(f"[DEBUG] Vendor reports raw: {vendor_reports_raw}")
    ...
    print(f"[DEBUG] Vendor reports processed: {vendor_reports}")
    print(f"[DEBUG] Current user role: {current_user.role}, employee_id: {current_user.employee_id}, location: {getattr(current_user, 'location', 'N/A')}")
    print(f"[DEBUG] Unit filter: {unit_filter}, Purpose filter: {purpose_filter}")
...
def vendor_report():
    ...
    query = "SELECT name as vendor_name, unit, food_licence_path, agreement_date FROM vendors WHERE (purpose IS NULL OR purpose = '' OR purpose NOT LIKE 'Outsider:%')"
    count_query = "SELECT COUNT(*) as count FROM vendors WHERE (purpose IS NULL OR purpose = '' OR purpose NOT LIKE 'Outsider:%')"
    params = []
    count_params = []
    where_conditions = []
    ...
    if current_user.role == 'Master Admin' and current_user.employee_id == 'a001':
        ...
        if unit_filter and unit_filter != current_user.location:
            ...
            return redirect(url_for('admin.dashboard'))
        unit_filter = current_user.location # Ensure the filter is set to their unit
        print(f"[DEBUG] Applied user location filter: {current_user.location}")

    if purpose_filter:
        where_conditions.append('purpose = %s')
        params.append(purpose_filter)
        count_params.append(purpose_filter)
        print(f"[DEBUG] Applied purpose filter: {purpose_filter}")
    
    # Keep the original logic but add debug output
    if unit_filter and not (current_user.role == 'Admin' and current_user.location and unit_filter == current_user.location):
        ...
        print(f"[DEBUG] Applied additional unit filter: {unit_filter}")

    if where_conditions:
        query += ' AND ' + ' AND '.join(where_conditions)
        count_query += ' AND ' + ' AND '.join(where_conditions)
        print(f"[DEBUG] Final query: {query}")
        print(f"[DEBUG] Final count_query: {count_query}")
    ...
    print(f"[DEBUG] Rendering template with vendor_items: {vendor_reports}")
    
    return render_template('admin/vendor_report.html',
                         vendor_items=vendor_reports,  # Changed from vendor_reports_raw to vendor_reports
                         units=units,
                         purposes=purposes,
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if original_vendor_name:
                ...
                if existing_vendor:
                    # Update existing vendor instead of inserting
                    print(f"[DEBUG] Vendor already exists, updating: {vendor_name}")
                    cur.execute("""
                        UPDATE vendors
                        SET unit = %s, food_licence_path = COALESCE(%s, food_licence_path), agreement_date = %s
                        WHERE name = %s
                    """, (unit, food_licence_path, agreement_date, vendor_name))
                    print(f"[DEBUG] Update query executed successfully")
                    flash('Vendor item updated successfully!', 'success')
                else:
                    # Add new vendor
